...
class Model extends Modele {
  ...
  // recherche  s'il existe un mel identique Ã  $mel
  //si $num=null recherche partout
  //si $num=integer recherche sauf pour $num (=si $num possède ce mel, c'est normal)
    $req='select count(*) as nb from '.$this->pref.'professeur where
          mel="'.$mel.'"';
    if (!is_null($num)) $req.=' and num!='.$num;
    $res=$this->mysql->execSQLRes($req);

    if ($res[0]["nb"]>0) { //ce mÃ©l est dÃ©jÃ  enregistrÃ© dans prof
      return true;
    } else {
      $req='select count(*) as nb from '.$this->pref.'etudiant where mel="'.$mel.'"';
      if (!is_null($num)) $req.=' and num!='.$num;
      $res=$this->mysql->execSQLRes($req);
      if ($res[0]["nb"]>0) { //ce mÃ©l est dÃ©jÃ  enregistrÃ© dans etud
        return true;
      } else {
        return false;
      }
  ...
  public function exporter($id) {
  	//export journalier format script sql en compressé
...
/*
  ...
  //$choix = g (groupe) a (annÃ©e) ou t (tout)
...
*/
  ...
  public function getBilanComp($processus,$groupe){
    ...
    //pour chaque Ã©lÃ¨ve recherche des validations par compÃ©tence
    for($i=0;$i<count($tb);$i++){
      $valeurs=array();
      ...
      for ($j=0;$j<count($tbc);$j++){
        ...
        /*
        if($tbr[0]["tt"]==0){ //pas validÃ©
          $valeurs[$j]=" "; // pas validÃ©
        }else{
          $valeurs[$j]="O";//validÃ© au moins une fois
        }
        */
  ...
  public function getBilanSitu($processus,$groupe){
    ...
    //rÃ©cupÃ¨re les Ã©lÃ¨ves du groupe :num,nom,prenom
    $tb=$this->getElevesGroupe($groupe);

    //rÃ©cupÃ¨re toutes les situations de l'atelier :id,nomen,lib,ln
    $tbs=$this->getSitusAtelier($atelier);

    //pour chaque Ã©lÃ¨ve, recherche de situations validÃ©es
    for($i=0;$i<count($tb);$i++){
      $valeurs=array();
  ...
  public function suppPersonne($vers,$num){
    if ($vers==1){// prof
      ...
      //laisse ses commentaires avec numProfesseur Ã  null
      ...
      $req='delete from '.$this->pref.'professeur
            where num='.$num;
      $this->mysql->execSQL($req);
    } else {//$vers=2 suppression de tout ce qui concerne l'Ã©tudiant
  ...
  public function setProf($nom,$prenom,$mel,$niveau,$smtp,$crypte){
    if ($this->existeMel($mel,null)) { //ce mÃ©l est dÃ©jÃ  enregistrÃ©
      return false;
    } else {
      if ($crypte && !$smtp)
      	$mdp=$this->mdpdefaut;
  ...
  public function majEleve($num,$nom,$pr,$mel,$mdp,$crypte,$smtp,$numexam) {
    ...
    $gr=$res[0]["numGroupe"];
    if ($this->existeMel($mel,$num)) { //ce mÃ©l est dÃ©jÃ  enregistrÃ©
      return false;
    } else {
  ...
  public function majProf($num,$nom,$pr,$mel,$mdp,$crypte,$smtp) {
    //if ($this->existeMel($mel,null)) { //ce mél est déjà enregistré//ERREUR
    if ($this->existeMel($mel,$num)) { //ce mél est déjà enregistré
      return false;
    } else {
      if ($mdp!=null){//veut changer mdp
      	if ($smtp)
  ...
  public function getAuthent($mel,$mdp){
    ...
    if (count($tb)==1){
      ...
      $tb=$this->mysql->execSQLRes($req);
      if (count($tb)==1){
        $tb[0]["niveau"]=3; //3=niveau Ã©lÃ¨ve
  ...
  public function getSitusClEl($i){
    ...
    for ($j=0;$j<count($tb);$j++){
      ...
      if ($tb1[0]["nb"]>0) {
        ...
        //dÃ©validation forcÃ©e pour Ã©liminer la situation fausse Ã  problÃ¨me :
        //  celle oÃ¹ valid=O et cloture=N
        if ($tb[$j]["cloture"]=="N") {
      ...
      //retourne les ref des situations comportant de compÃ©tences citÃ©es >1 fois
...
*/
  ...
  public function getSitus($id,$gr,$t){
    //le prof voit tous les Ã©tudiants de son groupe

    //$t=1 : situations rÃ©centes (dateModif > dateCommentaireLe+Récent) non commentées
    //$t=2 : situations rÃ©centes (dateModif > dateCommentaireLe+Récent)
    //$t=3 : non commentées
    //$t=4 : toutes
...
*/
  public function validSitu($refs,$prof,$refli){
    ...
    //foreach ($refli as $ref) echo 'prÃ©sente : '.$ref.'<br>';
    //foreach ($refs as $ref) echo 'Ã  valider : '.$ref.'<br>';


      //$refs = tableau des situ Ã  valider
      ...
      foreach($refli as $li){
        ...
        if (in_array($li, $refs, true)){//Ã  valider
  ...
  public function getTableauSyntheseNew($numeleve){//version qui met les activités communes dans le tableau
    ...
    $nba=0;  // compte nb total activités pour ces 5 processus = nb colonnes ds tableau synthèse
    $ida=array();  //tableau d'id d'activités à constuire
    for($i=0 ; $i<$nbp ; $i++){ //pour chaque processus récupère les activités
    ...
    $res=$this->mysql->execSQLRes($req);
    //récupère les situation ets /stage1 /stage2
  ...
  public function getTableauSyntheseNewV0($numeleve){
    ...
    $res=$this->mysql->execSQLRes($req);
    if (isset($res[0]["idParcours"])) $parc= $res[0]["idParcours"]; else $parc=0;
    if ($parc==0)//parcours encore indifférencié
  		$req="select id, nomenclature, libelle from ".$this->pref."processus order by 1";
  	else //SISR ou SLAM
    ...
    $tbet=$this->mysql->execSQLRes($req);
    $nbp=count($tbet);// 5=indifférencié, sinon 4
    $nba=0;  // compte nb total activités pour ces 4 ou 5 processus = nb colonnes ds tableau synthèse
    $ida=array();  //tableau d'id d'activités à constuire
    for($i=0 ; $i<$nbp ; $i++){ //pour chaque processus récupère les activités
      /*
  ...
  //retourne les seules activites thÃ©o traitÃ©es par l'etudiant
...
//premiere version en texte
    ...
    for ($i=0;$i<count($res);$i++){ //pour chaque situ citÃ©e
      ...
      for ($j=0;$j<count($resa);$j++){ //pour chaque activite citÃ©e
        //ajoute les compÃ©tences citÃ©es
        $refa=$resa[$j]["num"];
  ...
  public function getSesComps($numetud,$processus,$q){
    ...
    if ($processus!=0){
      if ($q!="t")
        //un processus, SOIT validÃ©es SOIT non validÃ©es
        $req="select distinct C.id, C.libelle, C.nomenclature from ".$this->pref."competence C, "
          ...
          and numEtudiant=".$numetud." order by 3";//pas terrible comme mÃ©thode...
      else
        //un processus, toutes
        $req="select distinct C.id, C.libelle, C.nomenclature from ".$this->pref."competence C, "
          .$this->pref."competencecitee CC, ".$this->pref."activitecitee AC, ".$this->pref."situation S, "
      ...
      if ($q!="t")
        //tous processus, SOIT validÃ©es SOIT non validÃ©es
        $req="select distinct C.id, C.libelle, C.nomenclature from ".$this->pref."competence C, "
          .$this->pref."competencecitee CC, ".$this->pref."activitecitee AC, ".$this->pref."situation S
  ...
  public function apprecie($numprof, $num, $com, $valid){
   ...
   //$e=t : les fichiers ne sont pas prÃ©supprimes
   ...
   //r_xxx_d_numeleve = rapport contenu supprimÃ© (en instance)
   //r_xxx_u_numeleve = rapport titre supprimÃ© (en instance)
   //b_xxx_d_numeleve = bilan contenu supprimÃ© (en instance)
   //b_xxx_u_numeleve = bilan titre supprimÃ© (en instance)
   //r_xxx_s_numeleve = rapport reponse prof supprimÃ© (en instance)
   //b_xxx_s_numeleve = bilan reponse prof supprimÃ© (en instance)
   ...
   //la suppression definitive (destruction) efface les fichiers du SE
    ...
    if ($t!=""){
      ...
      if ($iddoc==0){
      //recherche nÂ°suivant pour creation
      ...
      if ($e=="t") $f="c"; else $f="d";//on les conserve effacÃ©s si besoin
   ...
   //$type=c, t, r ou d, u, s contenu / titre / reponse / les mÃªmes supprimÃ©s
...
// modif ajoutée par cat legendre le 28/01/2015
/**
 * Fonction destinée à savoir si une situation donnée est propriété d'un étudiant donné
 *
 * @param le numéro d'étudiant recherché
 * @param le numéro de situation recherché
 * @return true si la situation est attachée à l'étudiant, false dans le cas contraire
 * 
 */
